/* user-list.c generated by valac 0.20.1, the Vala compiler
...
static gboolean _______lambda19_ (UserList* self);
static void user_list_test_call_set_remote_directory_servers (UserList* self);
static gboolean ________lambda19__gsource_func (gpointer self);
static gboolean ________lambda20_ (UserList* self);
static void user_list_test_call_remote_login_servers_updated (UserList* self);
static gboolean _________lambda20__gsource_func (gpointer self);
...
static gboolean ____lambda21_ (UserList* self);
...
static gboolean ___lambda25_ (UserList* self);
...
static gboolean user_list_query_directory_servers_co (UserListQueryDirectoryServersData* _data_) {
	...
	__catch32_g_io_error:
	{
		...
		g_debug ("user-list.vala:164: Calling GetServers on com.canonical.RemoteLogin db" \
...
static void user_list_set_remote_directory_servers (UserList* self, RemoteServer* server_list, int server_list_length1) {
	...
	_tmp59_ = _tmp54_;
	if (_tmp59_) {
		g_debug ("user-list.vala:254: removing manual login since we have a remote login" \
" entry");
...
static void __lambda24_ (UserList* self, GObject* obj, GAsyncResult* res) {
	...
	{
		GAsyncResult* _tmp0_;
		RemoteLoginService* _tmp1_ = NULL;
	...
	__catch33_g_io_error:
	{
		...
		g_debug ("user-list.vala:337: Getting the com.canonical.RemoteLogin dbus service" \
...
static gboolean user_list_remote_directory_respond_cb_co (UserListRemoteDirectoryRespondCbData* _data_) {
	...
	if (_data_->_tmp10_ == NULL) {
		g_debug ("user-list.vala:367: Something wrong happened in remote_directory_respo" \
"nd_cb. There was no password field");
	...
	if (_data_->_tmp11_ == NULL) {
		g_debug ("user-list.vala:372: Something wrong happened in remote_directory_respo" \
...
"*", 0, 0, &_data_->_inner_error_);
	...
	__catch34_g_regex_error:
	{
		...
		g_debug ("user-list.vala:391: Calling email regex match failed. Error: %s", _data_->_tmp34_);
		_g_error_free0 (_data_->e);
	}
	__finally34:
	if (_data_->_inner_error_ != NULL) {
	...
	if (!_data_->_tmp37_) {
			...
			if (_data_->_tmp46_) {
				...
				if (g_strcmp0 (_data_->_tmp49_, "password") == 0) {
					...
					if (g_strcmp0 (_data_->_tmp54_, "delay1") == 0) {
						...
						_data_->_server_list_size_ = _data_->server_list_length1;
						_data_->login_success = TRUE;
						g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 5000, ________lambda19__gsource_func, g_object_ref (_data_->self), g_object_unref);
					} else {
						...
						if (g_strcmp0 (_data_->_tmp59_, "delay2") == 0) {
							...
							_data_->_server_list_size_ = _data_->server_list_length1;
							_data_->login_success = TRUE;
							g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 5000, _________lambda20__gsource_func, g_object_ref (_data_->self), g_object_unref);
						} else {
		...
		__catch35_g_io_error:
		{
			...
			g_debug ("user-list.vala:452: Calling get_servers in com.canonical.RemoteLogin d" \
...
static void user_list_remote_login_respond_cb (UserList* self) {
	...
	if (_tmp6_) {
		...
		if (g_strcmp0 (_tmp16_, "delay") == 0) {
			g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 5000, _____lambda21__gsource_func, g_object_ref (self), g_object_unref);
		} else {
			greeter_list_authentication_complete_cb ((GreeterList*) self);
		}
										...
										if (g_strcmp0 (_data_->_tmp75_, "domain") == 0) {
											...
											if (g_strcmp0 (_data_->_tmp211_, "email") == 0) {
												...
												__catch36_g_io_error:
												{
													...
													g_debug ("user-list.vala:738: Calling get_cached_domains_for_server in com.canon" \
"ical.RemoteLogin dbus service failed. Error: %s", _data_->_tmp225_);
												...
												g_debug ("user-list.vala:749: Found field of type %s, don't know what to do with" \
" it", _data_->_tmp237_);
												_g_free0 (_data_->default_value);
												_g_object_unref0 (_data_->widget);
												_remote_server_field_free0 (_data_->_vala1_field);
...
static void user_list_prompt_box_login_cb (UserList* self) {
	...
	_tmp2_ = prompt_box_get_id (_tmp1_);
	_tmp3_ = _tmp2_;
	g_debug ("user-list.vala:849: Start session for %s", _tmp3_);
...
static void user_list_fill_list (UserList* self) {
	...
	if (_tmp1_) {
		...
		if (_tmp19_) {
			g_debug ("user-list.vala:970: Adding guest account entry");
...
static void user_list_user_added_cb (UserList* self, LightDMUser* user) {
	...
	_tmp5_ = _tmp4_;
	g_debug ("user-list.vala:988: Adding/updating user %s (%s)", _tmp2_, _tmp5_);
	_tmp6_ = user;
	...
	_tmp20_ = label;
	_tmp21_ = user;
	_tmp22_ = lightdm_user_get_background (_tmp21_);
	_tmp23_ = _tmp22_;
	_tmp24_ = user;
	_tmp25_ = lightdm_user_get_logged_in (_tmp24_);
	_tmp26_ = _tmp25_;
	_tmp27_ = user;
	_tmp28_ = lightdm_user_get_has_messages (_tmp27_);
	_tmp29_ = _tmp28_;
	_tmp30_ = user;
	_tmp31_ = lightdm_user_get_session (_tmp30_);
	_tmp32_ = _tmp31_;
	user_list_add_user (self, _tmp19_, _tmp20_, _tmp23_, _tmp26_, _tmp29_, _tmp32_);
	_g_free0 (label);
}
...
static void user_list_user_removed_cb (UserList* self, LightDMUser* user) {
	...
	_tmp1_ = lightdm_user_get_name (_tmp0_);
	_tmp2_ = _tmp1_;
	g_debug ("user-list.vala:999: Removing user %s", _tmp2_);
	_tmp3_ = user;
...
static void user_list_test_fill_list (UserList* self) {
	...
	if (_tmp17_) {
		g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) 1000, ____lambda25__gsource_func, g_object_ref (self), g_object_unref);
	}
	_tmp18_ = unity_greeter_singleton;
...
static void user_list_test_fill_remote_login_servers_duplicate_entries (UserList* self, RemoteServer** server_list, int* server_list_length1) {
	...
	RemoteServer _tmp38_;
	RemoteServer _tmp39_;
	RemoteServer _tmp40_ = {0};
	RemoteServer _tmp41_;
	g_return_if_fail (self != NULL);
	_tmp0_ = g_new0 (RemoteServer, 0);
	_vala_server_list = (_vala_RemoteServer_array_free (_vala_server_list, _vala_server_list_length1), NULL);
...
static gboolean user_list_add_test_entry (UserList* self) {
	...
	const gchar* _tmp27_;
	UserListTestEntry _tmp28_;
	const gchar* _tmp29_;
	const gchar* _tmp30_;
	UserListTestEntry _tmp31_;
	gboolean _tmp32_;
	UserListTestEntry _tmp33_;
	gboolean _tmp34_;
	UserListTestEntry _tmp35_;
	const gchar* _tmp36_;
	gint _tmp37_;
	g_return_val_if_fail (self != NULL, FALSE);
...
static void user_list_test_respond (UserList* self, const gchar* text) {
	...
	g_return_if_fail (text != NULL);
	_tmp0_ = text;
	g_debug ("user-list.vala:1396: response %s", _tmp0_);
	...
	if (_tmp4_ == ((0 != _tmp3_label0) ? _tmp3_label0 : (_tmp3_label0 = g_quark_from_static_string ("*other")))) {
		switch (0) {
			default:
			{
				...
				if (_tmp5_ == NULL) {
					...
					const gchar* _tmp9_ = NULL;
					_tmp6_ = text;
					g_debug ("user-list.vala:1402: username=%s", _tmp6_);
					_tmp7_ = text;
					_tmp8_ = g_strdup (_tmp7_);
					_g_free0 (((GreeterList*) self)->test_username);
					((GreeterList*) self)->test_username = _tmp8_;
					_tmp9_ = _ ("Password:");
					greeter_list_show_prompt_cb ((GreeterList*) self, _tmp9_, LIGHTDM_PROMPT_TYPE_SECRET);
				} else {
					const gchar* _tmp10_;
					_tmp10_ = text;
					((GreeterList*) self)->test_is_authenticated = g_strcmp0 (_tmp10_, "password") == 0;
					greeter_list_authentication_complete_cb ((GreeterList*) self);
				}
				break;
			}
		...
		switch (0) {
			...
			{
				gboolean _tmp11_;
				_tmp11_ = self->priv->test_prompted_sso;
				if (!_tmp11_) {
					const gchar* _tmp12_;
					_tmp12_ = text;
					if (g_strcmp0 (_tmp12_, "password") == 0) {
						g_debug ("user-list.vala:1417: prompt otp");
					...
					const gchar* _tmp13_;
					_tmp13_ = text;
					((GreeterList*) self)->test_is_authenticated = g_strcmp0 (_tmp13_, "otp") == 0;
					greeter_list_authentication_complete_cb ((GreeterList*) self);
				}
				break;
			}
		...
		switch (0) {
			...
			{
				const gchar* _tmp14_;
				_tmp14_ = self->priv->test_two_prompts_first;
				if (_tmp14_ == NULL) {
					const gchar* _tmp15_;
					gchar* _tmp16_;
					_tmp15_ = text;
					...
					self->priv->test_two_prompts_first = _tmp16_;
				} else {
					gboolean _tmp17_ = FALSE;
					const gchar* _tmp18_;
					gboolean _tmp20_;
					_tmp18_ = self->priv->test_two_prompts_first;
					if (g_strcmp0 (_tmp18_, "blue") == 0) {
						const gchar* _tmp19_;
						_tmp19_ = text;
						_tmp17_ = g_strcmp0 (_tmp19_, "password") == 0;
					} else {
						_tmp17_ = FALSE;
					}
					_tmp20_ = _tmp17_;
					((GreeterList*) self)->test_is_authenticated = _tmp20_;
					greeter_list_authentication_complete_cb ((GreeterList*) self);
				}
				break;
			}
		...
		switch (0) {
			default:
			{
				...
				if (_tmp21_ != NULL) {
					...
					const gchar* _tmp23_;
					_tmp22_ = text;
					_tmp23_ = self->priv->test_new_password;
					((GreeterList*) self)->test_is_authenticated = g_strcmp0 (_tmp22_, _tmp23_) == 0;
					greeter_list_authentication_complete_cb ((GreeterList*) self);
				} else {
					gboolean _tmp24_;
					_tmp24_ = self->priv->test_request_new_password;
					if (_tmp24_) {
						const gchar* _tmp25_;
						gchar* _tmp26_;
						_tmp25_ = text;
		...
		switch (0) {
			default:
			{
				...
				if (_tmp4_ != NULL) {
					...
					_tmp7_ = _ ("Password:");
					greeter_list_show_prompt_cb ((GreeterList*) self, _tmp7_, LIGHTDM_PROMPT_TYPE_SECRET);
				} else {
					const gchar* _tmp8_ = NULL;
					_tmp8_ = _ ("Username:");
					greeter_list_show_prompt_cb ((GreeterList*) self, _tmp8_, LIGHTDM_PROMPT_TYPE_QUESTION);
				}
				break;
			}
		}
...
"This is the last line of a multiple line message.", LIGHTDM_MESSAGE_TYPE_INFO);
				_tmp10_ = _ ("Password:");
				greeter_list_show_prompt_cb ((GreeterList*) self, _tmp10_, LIGHTDM_PROMPT_TYPE_SECRET);
				break;
			}
		}
...
" in these fine lands", LIGHTDM_MESSAGE_TYPE_INFO);
				_tmp11_ = _ ("Password:");
				greeter_list_show_prompt_cb ((GreeterList*) self, _tmp11_, LIGHTDM_PROMPT_TYPE_SECRET);
				break;
			}
		}
		...
		switch (0) {
			default:
			{
				...
				_tmp12_ = _ ("Password:");
				greeter_list_show_prompt_cb ((GreeterList*) self, _tmp12_, LIGHTDM_PROMPT_TYPE_SECRET);
				break;
			}
		}
		...
		switch (0) {
			default:
			{
				const gchar* _tmp13_ = NULL;
				const gchar* _tmp14_ = NULL;
				_tmp13_ = _ ("Favorite Color (blue):");
				greeter_list_show_prompt_cb ((GreeterList*) self, _tmp13_, LIGHTDM_PROMPT_TYPE_QUESTION);
				_tmp14_ = _ ("Password:");
				greeter_list_show_prompt_cb ((GreeterList*) self, _tmp14_, LIGHTDM_PROMPT_TYPE_SECRET);
				break;
			}
		}
	} else {
		switch (0) {
			default:
			{
				...
				_tmp15_ = _ ("Password:");
				greeter_list_show_prompt_cb ((GreeterList*) self, _tmp15_, LIGHTDM_PROMPT_TYPE_SECRET);
				break;
			}
		}
	}
...
static GObject * user_list_constructor (GType type, guint n_construct_properties, GObjectConstructParam * construct_properties) {
	...
	g_signal_connect_object ((GreeterList*) self, "entry-displayed-done", (GCallback) __user_list___lambda28__greeter_list_entry_displayed_done, self, 0);
	{
		...
		GdkPixbuf* _tmp5_;
		GdkPixbuf* _tmp6_;
		_tmp2_ = g_build_filename (PKGDATADIR, "message.png", NULL, NULL);
		_tmp3_ = _tmp2_;
	...
	__catch38_g_error:
	{
		...
		_tmp7_ = e;
		_tmp8_ = _tmp7_->message;
		g_debug ("user-list.vala:114: Error loading message image: %s", _tmp8_);
		_g_error_free0 (e);
	}
	__finally38:
	if (_inner_error_ != NULL) {
	...
	if (_tmp10_ != NULL) {
		...
		_tmp13_ = prompt_box_get_id (_tmp12_);
		_tmp14_ = _tmp13_;
		user_list_entry_selected_cb (self, _tmp14_);
	}
	greeter_list_connect_to_lightdm ((GreeterList*) self);
	_tmp16_ = unity_greeter_singleton;
	_tmp17_ = _tmp16_->test_mode;
	if (!_tmp17_) {
		UnityGreeter* _tmp18_;
		gboolean _tmp19_ = FALSE;
		_tmp18_ = unity_greeter_singleton;
		_tmp19_ = unity_greeter_show_remote_login_hint (_tmp18_);
		_tmp15_ = _tmp19_;
	} else {
		_tmp15_ = FALSE;
	}
